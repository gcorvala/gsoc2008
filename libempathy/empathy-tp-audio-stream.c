/*
 * stream.c - Source for EmpathyTpAudioStream
...
G_DEFINE_TYPE (EmpathyTpAudioStream, empathy_tp_audio_stream,
    G_TYPE_OBJECT);
...
struct _EmpathyTpAudioStreamPrivate
{
  TfStream *stream;
...
static GstElement *
empathy_tp_audio_stream_make_src_bin (EmpathyTpAudioStream *self);


static void empathy_tp_audio_stream_set_property  (GObject *object,
    guint property_id,
    const GValue *value,
    GParamSpec *pspec);

static void empathy_tp_audio_stream_get_property  (GObject *object,
    guint property_id,
    GValue *value,
    GParamSpec *pspec);
...
static void
empathy_tp_audio_stream_init (EmpathyTpAudioStream *self)
{
  EmpathyTpAudioStreamPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
      EMPATHY_TP_TYPE_AUDIO_STREAM, EmpathyTpAudioStreamPrivate);
...
static GObject *
empathy_tp_audio_stream_constructor (GType type,
    guint n_props,
    GObjectConstructParam *props)
{
  GObject *obj;
  EmpathyTpAudioStream *self = NULL;
  GstPad *src_pad = NULL;
  GstPad *sink_pad = NULL;
  GstElement *srcbin = NULL;

  obj = G_OBJECT_CLASS (empathy_tp_audio_stream_parent_class)->constructor (type, n_props, props);

  self = (EmpathyTpAudioStream *) obj;

  srcbin = empathy_tp_audio_stream_make_src_bin (self);
  if (!srcbin)
    {
...
{
  EmpathyTpAudioStream *self = EMPATHY_TP_AUDIO_STREAM (user_data);
  GstElement *bin = data;
...
static void
empathy_tp_audio_stream_dispose (GObject *object)
{
  EmpathyTpAudioStream *self = EMPATHY_TP_AUDIO_STREAM (object);
  ...
  if (G_OBJECT_CLASS (empathy_tp_audio_stream_parent_class)->dispose)
    G_OBJECT_CLASS (empathy_tp_audio_stream_parent_class)->dispose (object);
}

static void
empathy_tp_audio_stream_finalize (GObject *object)
{
  EmpathyTpAudioStream *self = EMPATHY_TP_AUDIO_STREAM (object);

  g_mutex_free (self->priv->mutex);

  if (G_OBJECT_CLASS (empathy_tp_audio_stream_parent_class)->finalize)
    G_OBJECT_CLASS (empathy_tp_audio_stream_parent_class)->finalize (object);
}
...
static void
empathy_tp_audio_stream_class_init (EmpathyTpAudioStreamClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GParamSpec *param_spec;

  g_type_class_add_private (klass, sizeof (EmpathyTpAudioStreamPrivate));
  object_class->dispose = empathy_tp_audio_stream_dispose;
  object_class->finalize = empathy_tp_audio_stream_finalize;
  object_class->constructor = empathy_tp_audio_stream_constructor;
  object_class->set_property = empathy_tp_audio_stream_set_property;
  object_class->get_property = empathy_tp_audio_stream_get_property;
...
static void
empathy_tp_audio_stream_set_property  (GObject *object,
    guint property_id,
    const GValue *value,
    GParamSpec *pspec)
{
...
static void
empathy_tp_audio_stream_get_property  (GObject *object,
    guint property_id,
    GValue *value,
    GParamSpec *pspec)
{
    ...
    GError **error)
{
  ...
  self = g_object_new (EMPATHY_TP_TYPE_AUDIO_STREAM,
      "stream", stream,
      "bin", bin,
      "pad", pad,
      NULL);
