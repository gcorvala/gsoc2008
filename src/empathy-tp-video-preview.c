...
G_DEFINE_TYPE (EmpathyTpVideoPreview, empathy_tp_video_preview,
    EMPATHY_TP_TYPE_VIDEO_SINK);

struct _EmpathyTpVideoPreviewPrivate
{
  GstPad *pad;
  GstElement *bin;
...
static void
empathy_tp_video_preview_init (EmpathyTpVideoPreview *self)
{
  EmpathyTpVideoPreviewPrivate *priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
      EMPATHY_TP_TYPE_VIDEO_PREVIEW, EmpathyTpVideoPreviewPrivate);
...
static GstElement *
make_sink (EmpathyTpVideoPreview *self)
{
...
static GObject *
empathy_tp_video_preview_constructor (GType type,
    guint n_props,
    GObjectConstructParam *props)
{
  GObject *obj;
  EmpathyTpVideoPreview *self = NULL;

  obj = G_OBJECT_CLASS (empathy_tp_video_preview_parent_class)->constructor (type, n_props, props);

  self = (EmpathyTpVideoPreview *) obj;
...
static void
empathy_tp_video_preview_dispose (GObject *object)
{
  EmpathyTpVideoPreview *self = EMPATHY_TP_VIDEO_PREVIEW (object);
  ...
  if (G_OBJECT_CLASS (empathy_tp_video_preview_parent_class)->dispose)
    G_OBJECT_CLASS (empathy_tp_video_preview_parent_class)->dispose (
        object);
}


static void
empathy_tp_video_preview_finalize (GObject *object)
{
  EmpathyTpVideoPreview *self = EMPATHY_TP_VIDEO_PREVIEW (object);

  g_mutex_free (self->priv->mutex);

  if (G_OBJECT_CLASS (empathy_tp_video_preview_parent_class)->finalize)
    G_OBJECT_CLASS (empathy_tp_video_preview_parent_class)->finalize (
        object);
}


static void
empathy_tp_video_preview_set_property  (GObject *object,
    guint property_id,
    const GValue *value,
    GParamSpec *pspec)
{
...
static void
empathy_tp_video_preview_get_property  (GObject *object,
    guint property_id,
    GValue *value,
    GParamSpec *pspec)
{
...
static void
empathy_tp_video_preview_class_init (EmpathyTpVideoPreviewClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GParamSpec *param_spec;

  g_type_class_add_private (klass, sizeof (EmpathyTpVideoPreviewPrivate));
  object_class->dispose = empathy_tp_video_preview_dispose;
  object_class->finalize = empathy_tp_video_preview_finalize;
  object_class->constructor = empathy_tp_video_preview_constructor;
  object_class->set_property = empathy_tp_video_preview_set_property;
  object_class->get_property = empathy_tp_video_preview_get_property;
    ...
    GError **error)
{
  ...
  self = g_object_new (EMPATHY_TP_TYPE_VIDEO_PREVIEW,
      "bin", bin,
      "is-preview", TRUE,
      NULL);
